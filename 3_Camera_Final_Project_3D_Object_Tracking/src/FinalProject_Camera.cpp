...
//        continue; // skips directly to the next image without processing what comes beneath
        ...
        if (dataBuffer.size() > 1) // wait until at least two images have been processed
        {
            ...
            //// STUDENT ASSIGNMENT-Part1/3 Authored by Vimal kumar V
            //// TASK FP.1 -> match list of 3D objects (vector<BoundingBox>) between current and previous frame (implement ->matchBoundingBoxes)
            map<int, int> bbBestMatches;
            matchBoundingBoxes(matches, bbBestMatches, *(dataBuffer.end()-2), *(dataBuffer.end()-1)); // associate bounding boxes between current and previous frame using keypoint matches
            //// EOF STUDENT ASSIGNMENT-Part1/3 Authored by Vimal kumar V
...
for(auto elem :bbBestMatches)
  cout<<elem.first<<"maps to"<<elem.second<<endl;
            ...
            for (auto it1 = (dataBuffer.end() - 1)->bbMatches.begin(); it1 != (dataBuffer.end() - 1)->bbMatches.end(); ++it1)
            {
                ...
                {
                    //// STUDENT ASSIGNMENT-Part2/3 Authored by Vimal kumar V
                    //// TASK FP.2 -> compute time-to-collision based on Lidar data (implement -> computeTTCLidar)
                    double ttcLidar; 
                    computeTTCLidar(prevBB->lidarPoints, currBB->lidarPoints, sensorFrameRate, ttcLidar);
                    //// EOF STUDENT ASSIGNMENT--Part2/3 Authored by Vimal kumar V

                    //// STUDENT ASSIGNMENT-Part3/3 Authored by Vimal kumar V
                    //// TASK FP.3 -> assign enclosed keypoint matches to bounding box (implement -> clusterKptMatchesWithROI)
                    //// TASK FP.4 -> compute time-to-collision based on camera (implement -> computeTTCCamera)
                    double ttcCamera;
                    clusterKptMatchesWithROI(*currBB, (dataBuffer.end() - 2)->keypoints, (dataBuffer.end() - 1)->keypoints, (dataBuffer.end() - 1)->kptMatches);                    
                    computeTTCCamera((dataBuffer.end() - 2)->keypoints, (dataBuffer.end() - 1)->keypoints, currBB->kptMatches, sensorFrameRate, ttcCamera);
                    //// EOF STUDENT ASSIGNMENT-Part3/3 Authored by Vimal kumar V
