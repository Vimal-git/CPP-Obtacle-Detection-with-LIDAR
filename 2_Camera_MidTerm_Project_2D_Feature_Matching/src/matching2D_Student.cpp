...
void matchDescriptors(std::vector<cv::KeyPoint> &kPtsSource, std::vector<cv::KeyPoint> &kPtsRef, cv::Mat &descSource, cv::Mat &descRef,std::vector<cv::DMatch> &matches, std::string descriptorType, std::string matcherType, std::string selectorType)
{
    ...
    {
        int normType = cv::NORM_HAMMING;//NORM_L2,NORM_HAMMING
      	//int normType = cv::NORM_L2;
      
        matcher = cv::BFMatcher::create(normType, crossCheck);
      	cout<<"BF matching  with ";
    }
    else if (matcherType.compare("MAT_FLANN") == 0)
    {
        // ...
      //woraround to fix bug
    ...
    { // nearest neighbor (best match)
         ...
         matcher->match(descSource, descRef, matches); // Finds the best match for each descriptor in desc1
      	 ...
      	 cout << "Nearest Neighbour matching n =" << matches.size() << " keypoints in " << 1000 * t / 1.0 << " ms" << endl;
    }
    ...
    { // k nearest neighbors (k=2)
        ...
        // ...
      	...
      	matcher->knnMatch(descSource, descRef,knn_matches,2);//knn matching
      //descriptor filtering
      	const float ratio_thresh = 0.8f;//threshold for descriptor filtering    
    ...
    t = ((double)cv::getTickCount() - t) / 	cv::getTickFrequency();
      	 cout << "KNN(k=2,threshold = 0.8) matching n =" << matches.size() << " keypoints in " << 1000 * t / 1.0 << " ms" << endl;
...
void descKeypoints(vector<cv::KeyPoint> &keypoints, cv::Mat &img, cv::Mat &descriptors, string descriptorType)
{
    ...
    else
    {
    	...
    	{
          int nfeatures = 500;//500,1000,2000
    	...
    	{
          cv::AKAZE::DescriptorType descriptor_type = 			cv::AKAZE::DESCRIPTOR_MLDB;//DESCRIPTOR_KAZE, DESCRIPTOR_KAZE_UPRIGHT, DESCRIPTOR_MLDB or DESCRIPTOR_MLDB_UPRIGHT
  		 int descriptor_size = 0;
  		 int descriptor_channels = 3;
  		 float threshold = 0.001f;
  		 int nOctaves = 4;
  		 int nOctaveLayers = 4;
  		 cv::KAZE::DiffusivityType diffusivity = cv::KAZE::DIFF_PM_G2;// DIFF_PM_G1, DIFF_PM_G2, DIFF_WEICKERT or DIFF_CHARBONNIER
          ...
          {//only store points above a threshold
            ...
            for(auto it=keypoints.begin();it!=keypoints.end();++it)
            {
              ...
              if(kptOverlap>maxOverlap)
              {
               ...
               {
                 *it = newKeyPoint;//replace key point
                 break;//quit loop over key points
               }
              }
            }
        	if(!bOverlap)
...
void detKeypointsOrb(std::vector<cv::KeyPoint> &keypoints, cv::Mat &img, bool bVis)
  ...
  //Detect key points
  double t = (double)cv::getTickCount();
  cv::Ptr<cv::FeatureDetector> detector =cv::ORB::create(nfeatures,scaleFactor,nlevels,edgeThreshold,firstLevel,WTA_K,scoreType,patchSize);
  detector->detect(img,keypoints);
  t = ((double)cv::getTickCount() - t) / cv::getTickFrequency();
...
void detKeypointsAkaze(std::vector<cv::KeyPoint> &keypoints, cv::Mat &img, bool bVis)
  {
  //Detector parameters(Ref:opencv documentation)
  cv::AKAZE::DescriptorType descriptor_type = cv::AKAZE::DESCRIPTOR_MLDB;//DESCRIPTOR_KAZE, DESCRIPTOR_KAZE_UPRIGHT, DESCRIPTOR_MLDB or DESCRIPTOR_MLDB_UPRIGHT
  int descriptor_size = 0;
  int descriptor_channels = 3;
  float threshold = 0.001f;
